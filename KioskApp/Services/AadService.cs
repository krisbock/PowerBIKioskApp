            ...
            if (azureAd.Value.AuthenticationMode.Equals("masteruser", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                catch (MsalUiRequiredException)
                {
                    throw new InvalidOperationException(
                        "MasterUser authentication no longer supports username/password fallback because the MSAL ROPC flow is deprecated. " +
                        "Switch to ServicePrincipal authentication mode. See https://aka.ms/msal-ropc-migration for guidance.");
                }
            }
